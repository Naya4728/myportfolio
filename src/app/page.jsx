...
export default function PortfolioWebsite() {
  ...
  return (
    <div className={`${themeDark ? "bg-gray-900 text-gray-100" : "bg-white text-gray-900"} min-h-screen transition-colors duration-500`}>
      ...
      <main className="scroll-smooth">
        ...
        <section id="about" className="max-w-6xl mx-auto px-6 py-24 scroll-mt-16">
          <motion.h2 className="text-4xl font-semibold mb-6" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
            About Me
          </motion.h2>
          <motion.p className="text-gray-300/90 mb-6" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            {data.intro}
          </motion.p>
          <motion.div className="flex flex-wrap gap-3" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ staggerChildren: 0.1 }} viewport={{ once: true }}>
        ...
        <section id="projects" className="max-w-6xl mx-auto px-6 py-24 scroll-mt-16">
          ...
          <div className="grid md:grid-cols-2 gap-10">
            {data.projects.map((project) => (
              <motion.div
                key={project.id}
                className="relative group h-64 bg-gray-800/20 border border-gray-700/50 rounded-2xl p-6 flex flex-col justify-between hover:bg-gray-800/40 transition"
              ...
              >
                <div>
                  ...
                  <p className="text-gray-300/90">{project.desc}</p>
                </div>
                ...
                <a href={project.link} target="_blank" rel="noreferrer" className="mt-4 text-sm underline text-pink-400 hover:text-pink-600">
                  View Project
                </a>
              </motion.div>
            ))}
        ...
        <section id="contact" className="max-w-6xl mx-auto px-6 py-24 scroll-mt-16">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center gap-6 text-gray-300/90">
            <a href={`mailto:${data.contact.email}`} className="flex items-center gap-2 text-purple-400 hover:underline">
              <FaEnvelope /> {data.contact.email}
            </a>
            <a href={data.contact.github} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-purple-400 hover:underline">
              <FaGithub /> GitHub
            </a>
            <a href={data.contact.linkedin} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-purple-400 hover:underline">
              <FaLinkedin /> LinkedIn
            </a>
          </div>
        </section>
